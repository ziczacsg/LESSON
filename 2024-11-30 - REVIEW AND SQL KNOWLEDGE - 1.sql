...
-- mô tả cấu trúc của 1 bảng dữ liệu hoàn chỉnh: RISK_PORTFOLIA.dbo.DIM_CUSTOMER_PROFILE
---dbo: schema mặc định trong SQL - còn gọi là default
--"DIM_CUSTOMER_PROFILE": là TABLE NAME
---Dấu "..": là viết tắt của chữ "dbo"

--Cách kiểm tra cấu trúc bảng trong Microsoft SQL Server
--INFORMATION_SCHEMA.TABLES: Bảng truy vấn thông tin của 1 DATABASE
...
--CÂU LỆNH 1: 
--Định nghĩa: "Trường dữ liệu" hay "Cột dữ liệu" hoặc "Biến" là 1 - Attribute
--Định nghĩa: "Bản Ghi" hay "Dòng dữ liệu là một -Record

SELECT * FROM INFORMATION_SCHEMA.TABLES

SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME LIKE '%INCOME%'
...
SELECT 
         TOP 100 *--LẤY TOP TRONG TRƯỜNG HỢP KHÔNG CÓ ĐIỀU KIỆN GIỐNG VẦY THÌ CHỈ LẤY RANDOM
...
SELECT 
	 ...
	 ,RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE.CUSTOMERID
...
SELECT * FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE  WHERE PROVINCE <= N'Hà Nội' ORDER BY PROVINCE ASC  --(ASCENDING): CÓ Ý NGHĨA LÀ SORTING TĂNG DẦN, VÀ NẾU MUỐN SORTING TĂNG DẦN MÌNH ĐẶT ASC HOẶC "KHÔNG ĐẶT" ĐIỀU ĐƯỢC --> CÓ Ý NGHĨA TĂNG DẦN  --DẠNG LỌC SỬ DỤNG KÝ HIỆU TOÁN HỌC THEO KÝ TỰ ">" & "<" 
...
SELECT PROVINCE,GENDER FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE PROVINCE = N'Hà Nội' AND GENDER = N'Nữ'      --SỬ DỤNG AND CHO TỪ 2 ĐIỀU KIỆN TRỞ LÊN, KHÁC CỘT

SELECT PROVINCE,INCOME_TYPE FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE INCOME_TYPE LIKE '%Business%' --Wildcard          --SỬ DỤNG LIKE ĐỂ LỌC 1 ĐIỀU KIỆN VÀ GIỐNG ĐIỀU KIỆN "=", KẾT HỢP 2 DẤU % ĐỂ LẤY RA KÝ TỰ NẰM TRONG CHUỖI KÝ TỰ MÌNH CẦN LỌC, % BÊN TRÁI VÀ % BÊN PHẢI NGHĨA LÀ BỎ KÝ TỰ Ở BÊN TRÁI VÀ PHẢI ĐỂ LẤY ĐÚNG KÝ TỰ CẦN LỌC

SELECT LOAN_ACCOUNT_NUMBER,CUSTOMER_NAME FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE CUSTOMER_NAME LIKE N'Lê%'                      --SỬ DỤNG LIKE KẾT HỢP % LỌC THEO ĐIỀU KIỆN TỪ TRÁI QUA PHẢI VÀ BỎ QUA ĐIỀU KIỆN LỌC BÊN PHẢI

SELECT LOAN_ACCOUNT_NUMBER,PHONE_NUMBER,CUSTOMER_NAME FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE PHONE_NUMBER LIKE '%11'                         --SỬ DỤNG LIKE KẾT HỢP % LỌC THEO ĐIỀU KIỆN TỪ PHẢI QUA TRÁI VÀ BỎ QUA ĐIỀU KIỆN LỌC BÊN TRÁI
...
SELECT CUSTOMER_NAME,LOAN_AMOUNT FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE LOAN_AMOUNT BETWEEN 50 * POWER(10,6) AND 100 * POWER(10,6) --BETWEEN...AND TƯƠNG ĐƯƠNG VỚI ">= & <="


SELECT * FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE DISBURSEMENT_DATE BETWEEN '2020-01-02' AND '2023-12-31'
...
SELECT LOAN_ACCOUNT_NUMBER,CUSTOMER_NAME,DISBURSEMENT_DATE,LOAN_AMOUNT FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE DAY(DISBURSEMENT_DATE) = 31 AND MONTH(DISBURSEMENT_DATE) = 12 AND YEAR(DISBURSEMENT_DATE) = 2023


SELECT LOAN_ACCOUNT_NUMBER,CUSTOMER_NAME,DISBURSEMENT_DATE,LOAN_AMOUNT FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE DISBURSEMENT_DATE = '2023-12-31'

--LẤY RA KH ĐƯỢC GIẢI NGÂN VÀO NĂM 2023 VÀ NĂM 2022

SELECT LOAN_ACCOUNT_NUMBER,CUSTOMER_NAME,DISBURSEMENT_DATE,LOAN_AMOUNT FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE YEAR(DISBURSEMENT_DATE) = 2022
...
--LẤY DÙM TÔI TOP 10 NHỮNG KHOẢN VAY DƯỚI 20TR 

SELECT TOP 10 LOAN_ACCOUNT_NUMBER,CUSTOMER_NAME,FORMAT(LOAN_AMOUNT,'#,##0;(#,##0)') AS LOAN_AMOUNT FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE WHERE LOAN_AMOUNT < 50000000 ORDER BY LOAN_AMOUNT DESC 
...
SELECT 
	  ...
	  ,LOAN_AMOUNT 
FROM RISK_PORTFOLIO..DIM_CUSTOMER_PROFILE 
WHERE DATEPART(QUARTER,DISBURSEMENT_DATE) < 3 AND YEAR(DISBURSEMENT_DATE) = 2022
